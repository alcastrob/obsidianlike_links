...
Corregido (2026-08-28): en una nota con acento en el nombre (p. ej. `👶 José Javier Montes Romero (JJ).md`), Enlaces entrantes solo mostraba 1 de decenas de enlaces reales. Causa: el nombre de archivo en disco estaba en forma Unicode descompuesta (NFD, típico de archivos originados en macOS) y los wikilinks en forma precompuesta (NFC), así que la comparación fallaba pese a verse idénticos. Ahora todos los nombres de nota se comparan tras normalizar a NFC. Ver detalle técnico en `CLAUDE.md`.
